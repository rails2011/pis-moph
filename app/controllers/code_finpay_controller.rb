class CodeFinpayController < ApplicationController
        ...
        def read
                ...
                rs = CodeFinpay.find(:all, :conditions => case_search, :limit => limit, :offset => start, :order => "id")
                return_data = Hash.new()
                return_data[:totalCount] = CodeFinpay.count(:all , :conditions => case_search)
	...
	def create
                ...
                if records.type == Hash
                        if records["fcode"] == "" and records["finname"] == "" and records["use_status"] == ""
                                return_data = Hash.new()
                                return_data[:success] = false
                                render :text => return_data.to_json, :layout => false
                        else
                                count_check = CodeFinpay.count(:all, :conditions => "fcode = '#{records["fcode"]}'")
                                if count_check > 0
                                        return_data = Hash.new()
                                        return_data[:success] = false
                                        ...
                                        store = CodeFinpay.create(records)
                                        return_data = Hash.new()
                                        return_data[:success] = true
                                        return_data[:records] = {
                                                :id         => store[:id],
                                                :fcode      => store[:fcode],
                                                :finname    => store[:finname],
	...
	def update
                records = ActiveSupport::JSON.decode(params[:records])
                if records.type == Hash
                        count_check = CodeFinpay.count(:all , :conditions => "fcode = '#{records["fcode"]}' and id <> '#{records["id"]}' ")
                        if count_check > 0
                                return_data = Hash.new()
                                return_data[:success] = false
                                return_data[:msg] = "มีรหัสนี้แล้ว"
                                ...
                                update_record = CodeFinpay.find(:all , :conditions => "id = '#{records["id"]}'")[0]
                                update_record.fcode = records["fcode"]
                                update_record.finname = records["finname"]
	...
	def report
                ...
                @records = CodeFinpay.find(:all, :conditions => case_search, :order => "fcode")
        end
end
