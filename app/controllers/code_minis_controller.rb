class CodeMinisController < ApplicationController
        ...
        def read
                ...
                rs = CodeMinis.find(:all, :conditions => case_search, :limit => limit, :offset => start, :order => "mincode")
                return_data = Hash.new()
                return_data[:totalCount] = CodeMinis.count(:all , :conditions => case_search)
        ...
        def create
                ...
                if records.type == Hash
                        if records["id"] == "" and records["mincode"] == "" and records["minname"] == "" and records["stdcode"] == "" and records["use_status"] == ""
                                return_data = Hash.new()
                                return_data[:success] = false
                                render :text => return_data.to_json, :layout => false
                        else
                                count_check = CodeMinis.count(:all, :conditions => "mincode = '#{records["mincode"]}'")
                                if count_check > 0
                                        return_data = Hash.new()
                                        return_data[:success] = false
                                        ...
                                        store = CodeMinis.create(records)
                                        return_data = Hash.new()
                                        return_data[:success] = true
                                        return_data[:records] = {
                                                :id => store[:id],
                                                :mincode => store[:mincode],
        ...
        def update
                records = ActiveSupport::JSON.decode(params[:records])
                if records.type == Hash
                        count_check = CodeMinis.count(:all , :conditions => "mincode = '#{records["mincode"]}' and id <> '#{records["id"]}' ")
                        if count_check > 0
                                return_data = Hash.new()
                                return_data[:success] = false
                                return_data[:msg] = "มีรหัสตำแหน่งนี้แล้ว"
                                ...
                                update_record = CodeMinis.find(:all , :conditions => "id = '#{records["id"]}'")[0]
                                update_record.mincode = records["mincode"]
                                update_record.minname = records["minname"]
        ...
        def report
                ...
                @records = CodeMinis.find(:all, :conditions => case_search)
        end
        ...
        def genres
          case_search = ''
          if !params[:query].nil?
            case_search = "minname like '%#{params[:query]}%'"
          end
          records = CodeMinis.find(:all,:conditions =>case_search)
          return_data = Hash.new()
          return_data[:success] = true
