class CodePositionController < ApplicationController
        ...
        def read
                ...
                rs = CodePosition.find(:all, :conditions => case_search, :limit => limit, :offset => start, :order => "poscode")
                return_data = Hash.new()
                return_data[:totalCount] = CodePosition.count(:all , :conditions => case_search)
        ...
        def create
                ...
                if records.type == Hash
                        if records["id"] == "" and records["poscode"] == "" and records["shortpre"] == "" and records["longpre"] == "" and records["posname"] == "" and records["stdcode"] == "" and records["use_status"] == ""
                                return_data = Hash.new()
                                return_data[:success] = false
                                render :text => return_data.to_json, :layout => false
                        else
                                count_check = CodePosition.count(:all, :conditions => "poscode = '#{records["poscode"]}'")
                                if count_check > 0
                                        return_data = Hash.new()
                                        return_data[:success] = false
                                        ...
                                        store = CodePosition.create(records)
                                        return_data = Hash.new()
        ...
        def update
                records = ActiveSupport::JSON.decode(params[:records])
                if records.type == Hash
                        count_check = CodePosition.count(:all , :conditions => "poscode = '#{records["poscode"]}' and id <> '#{records["id"]}' ")
                        if count_check > 0
                                return_data = Hash.new()
                                return_data[:success] = false
                                return_data[:msg] = "มีรหัสตำแหน่งนี้แล้ว"
                                ...
                                update_record = CodePosition.find(:all , :conditions => "id = '#{records["id"]}'")[0]
                                update_record.poscode = records["poscode"]
                                update_record.shortpre = records["shortpre"]
                                update_record.longpre = records["longpre"]
                                update_record.posname = records["posname"]
        ...
        def report
                ...
                @records = CodePosition.find(:all, :conditions => case_search, :order => "poscode")
        end
        def genres
          case_search = ''
          if !params[:query].nil?
            case_search = "posname like '%#{params[:query]}%'"
          end
          records = CodePosition.find(:all,:conditions =>case_search)
          return_data = Hash.new()
          return_data[:success] = true
