class CodeExecutiveController < ApplicationController
        ...
        def read
                ...
                rs = CodeExecutive.find(:all, :conditions => case_search, :limit => limit, :offset => start, :order => "excode")
                return_data = Hash.new()
                return_data[:totalCount] = CodeExecutive.count(:all , :conditions => case_search)
        ...
        def create
                ...
                if records.type == Hash
                        if records["id"] == "" and records["excode"] == "" and records["shortpre"] == "" and records["longpre"] == "" and records["exname"] == "" and records["stdcode"] == "" and records["use_status"] == ""
                                return_data = Hash.new()
                                return_data[:success] = false
                                render :text => return_data.to_json, :layout => false
                        else
                                count_check = CodeExecutive.count(:all, :conditions => "excode = '#{records["excode"]}'")
                                if count_check > 0
                                        return_data = Hash.new()
                                        return_data[:success] = false
                                        ...
                                        store = CodeExecutive.create(records)
                                        return_data = Hash.new()
        ...
        def update
                records = ActiveSupport::JSON.decode(params[:records])
                if records.type == Hash
                        count_check = CodeExecutive.count(:all , :conditions => "excode = '#{records["excode"]}' and id <> '#{records["id"]}' ")
                        if count_check > 0
                                return_data = Hash.new()
                                return_data[:success] = false
                                return_data[:msg] = "มีรหัสตำแหน่งนี้แล้ว"
                                ...
                                update_record = CodeExecutive.find(:all , :conditions => "id = '#{records["id"]}'")[0]
                                update_record.excode = records["excode"]
                                update_record.shortpre = records["shortpre"]
                                update_record.longpre = records["longpre"]
                                update_record.exname = records["exname"]
        ...
        def report
                ...
                @records = CodeExecutive.find(:all, :conditions => case_search)
        end
        def genres
          case_search = ''
          if !params[:query].nil?
            case_search = "exname like '%#{params[:query]}%'"
          end
          records = CodeExecutive.find(:all,:conditions =>case_search)
          return_data = Hash.new()
          return_data[:success] = true
