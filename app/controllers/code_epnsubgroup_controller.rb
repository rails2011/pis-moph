class CodeEpnsubgroupController < ApplicationController
        ...
        def read
                ...
                rs = CodeEpnsubgroup.find(:all, :conditions => case_search, :limit => limit, :offset => start, :order => "sgcode")
                return_data = Hash.new()
                return_data[:totalCount] = CodeEpnsubgroup.count(:all , :conditions => case_search)
        ...
        def create
                ...
                if records.type == Hash
                        if records["id"] == "" and records["sgcode"] == "" and records["sgname"] == "" and records["stdcode"] == "" and records["use_status"] == ""
                                return_data = Hash.new()
                                return_data[:success] = false
                                render :text => return_data.to_json, :layout => false
                        else
                                count_check = CodeEpnsubgroup.count(:all, :conditions => "sgcode = '#{records["sgcode"]}'")
                                if count_check > 0
                                        return_data = Hash.new()
                                        return_data[:success] = false
                                        ...
                                        store = CodeEpnsubgroup.create(records)
                                        return_data = Hash.new()
        ...
        def update
                records = ActiveSupport::JSON.decode(params[:records])
                if records.type == Hash
                        count_check = CodeEpnsubgroup.count(:all , :conditions => "sgcode = '#{records["sgcode"]}' and id <> '#{records["id"]}' ")
                        if count_check > 0
                                return_data = Hash.new()
                                return_data[:success] = false
                                ...
                                update_record = CodeEpnsubgroup.find(:all , :conditions => "id = '#{records["id"]}'")[0]
                                update_record.sgcode = records["sgcode"]
                                update_record.sgname = records["sgname"]
        ...
        def report
                ...
                @records = CodeEpnsubgroup.find(:all, :conditions => case_search)
        end
end
