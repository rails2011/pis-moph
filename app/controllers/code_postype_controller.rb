class CodePostypeController < ApplicationController
        ...
        def read
                ...
                rs = CodePostype.find(:all, :conditions => case_search, :limit => limit, :offset => start, :order => "ptcode")
                return_data = Hash.new()
                return_data[:totalCount] = CodePostype.count(:all , :conditions => case_search)
        ...
        def create
                ...
                if records.type == Hash
                        if records["id"] == "" and records["ptcode"] == "" and records["ptname"] == "" and records["shortmn"] == "" and records["stdcode"] == "" and records["use_status"] == ""
                                return_data = Hash.new()
                                return_data[:success] = false
                                render :text => return_data.to_json, :layout => false
                        else
                                count_check = CodePostype.count(:all, :conditions => "ptcode = '#{records["ptcode"]}'")
                                if count_check > 0
                                        return_data = Hash.new()
                                        return_data[:success] = false
                                        ...
                                        store = CodePostype.create(records)
                                        return_data = Hash.new()
        ...
        def update
                records = ActiveSupport::JSON.decode(params[:records])
                if records.type == Hash
                        count_check = CodePostype.count(:all , :conditions => "ptcode = '#{records["ptcode"]}' and id <> '#{records["id"]}' ")
                        if count_check > 0
                                return_data = Hash.new()
                                return_data[:success] = false
                                ...
                                update_record = CodePostype.find(:all , :conditions => "id = '#{records["id"]}'")[0]
                                update_record.ptcode = records["ptcode"]
                                update_record.ptname = records["ptname"]
                                update_record.shortmn = records["shortmn"]
        ...
        def report
                ...
                @records = CodePostype.find(:all, :conditions => case_search)
        end
        
        def genres
          case_search = ''
          if !params[:query].nil?
            case_search = "ptname like '%#{params[:query]}%'"
          end
          records = CodePostype.find(:all,:conditions =>case_search)
          return_data = Hash.new()
          return_data[:success] = true
