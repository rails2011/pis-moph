class CodeMajorController < ApplicationController
        ...
        def read
                ...
                rs = CodeMajor.find(:all, :conditions => case_search, :limit => limit, :offset => start, :order => "macode")
                return_data = Hash.new()
                return_data[:totalCount] = CodeMajor.count(:all , :conditions => case_search)
        ...
        def create
                ...
                if records.type == Hash
                        if records["id"] == "" and records["macode"] == "" and records["major"] == "" and records["stdcode"] == "" and records["use_status"] == ""
                                return_data = Hash.new()
                                return_data[:success] = false
                                render :text => return_data.to_json, :layout => false
                        else
                                count_check = CodeMajor.count(:all, :conditions => "macode = '#{records["macode"]}'")
                                if count_check > 0
                                        return_data = Hash.new()
                                        return_data[:success] = false
                                        ...
                                        store = CodeMajor.create(records)
                                        return_data = Hash.new()
        ...
        def update
                records = ActiveSupport::JSON.decode(params[:records])
                if records.type == Hash
                        count_check = CodeMajor.count(:all , :conditions => "macode = '#{records["macode"]}' and id <> '#{records["id"]}' ")
                        if count_check > 0
                                return_data = Hash.new()
                                return_data[:success] = false
                                return_data[:msg] = "มีรหัสตำแหน่งนี้แล้ว"
                                ...
                                update_record = CodeMajor.find(:all , :conditions => "id = '#{records["id"]}'")[0]
                                update_record.macode = records["macode"]
                                update_record.major = records["major"]
        ...
        def report
                ...
                @records = CodeMajor.find(:all, :conditions => case_search)
        end
        
        def genres
          case_search = ''
          if !params[:query].nil?
            case_search = "major like '%#{params[:query]}%'"
          end
          records = CodeMajor.find(:all,:conditions =>case_search)
          return_data = Hash.new()
          return_data[:success] = true
